...
public class WorldInfoGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<WorldInfoSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<WorldInfoComponent> ghostWorldInfoComponentType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostWorldInfoComponentArray = chunk.GetNativeArray(ghostWorldInfoComponentType);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
...
#endif
                WorldInfoSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostWorldInfoComponent = ghostWorldInfoComponentArray[entityIndex];
                ghostWorldInfoComponent.WorldSeed = snapshotData.GetWorldInfoComponentWorldSeed(deserializerState);
                ghostWorldInfoComponent.RegionSize = snapshotData.GetWorldInfoComponentRegionSize(deserializerState);
                ghostWorldInfoComponent.DifficultyLevel = snapshotData.GetWorldInfoComponentDifficultyLevel(deserializerState);
                ghostWorldInfoComponentArray[entityIndex] = ghostWorldInfoComponent;
            }
        }
    }
    [BurstCompile]
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<WorldInfoSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<WorldInfoComponent>(),
            },
        ...
        {
            All = new []{
                ComponentType.ReadOnly<WorldInfoSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<WorldInfoComponent>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<WorldInfoSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<WorldInfoSnapshotData>(true),
                ...
                ghostWorldInfoComponentType = GetArchetypeChunkComponentType<WorldInfoComponent>(),
...
public partial class WorldInfoGhostSpawnSystem : DefaultGhostSpawnSystem<WorldInfoSnapshotData>
{
}
