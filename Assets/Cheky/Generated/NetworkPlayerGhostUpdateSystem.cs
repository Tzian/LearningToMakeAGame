...
public class NetworkPlayerGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<NetworkPlayerSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<NetworkPlayerComponent> ghostNetworkPlayerComponentType;
        public ArchetypeChunkComponentType<PlayerInfoComponent> ghostPlayerInfoComponentType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostNetworkPlayerComponentArray = chunk.GetNativeArray(ghostNetworkPlayerComponentType);
            var ghostPlayerInfoComponentArray = chunk.GetNativeArray(ghostPlayerInfoComponentType);
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
...
#endif
                NetworkPlayerSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostNetworkPlayerComponent = ghostNetworkPlayerComponentArray[entityIndex];
                var ghostPlayerInfoComponent = ghostPlayerInfoComponentArray[entityIndex];
                var ghostRotation = ghostRotationArray[entityIndex];
                var ghostTranslation = ghostTranslationArray[entityIndex];
                ghostNetworkPlayerComponent.PlayerId = snapshotData.GetNetworkPlayerComponentPlayerId(deserializerState);
                ghostPlayerInfoComponent.PlayerId = snapshotData.GetPlayerInfoComponentPlayerId(deserializerState);
                ghostPlayerInfoComponent.AvatarModelName = snapshotData.GetPlayerInfoComponentAvatarModelName(deserializerState);
                ghostPlayerInfoComponent.PlayerName = snapshotData.GetPlayerInfoComponentPlayerName(deserializerState);
                ...
                ghostNetworkPlayerComponentArray[entityIndex] = ghostNetworkPlayerComponent;
                ghostPlayerInfoComponentArray[entityIndex] = ghostPlayerInfoComponent;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<NetworkPlayerSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<NetworkPlayerComponent>(),
                ComponentType.ReadWrite<PlayerInfoComponent>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<NetworkPlayerSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<NetworkPlayerSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<NetworkPlayerSnapshotData>(true),
                ...
                ghostNetworkPlayerComponentType = GetArchetypeChunkComponentType<NetworkPlayerComponent>(),
                ghostPlayerInfoComponentType = GetArchetypeChunkComponentType<PlayerInfoComponent>(),
...
public partial class NetworkPlayerGhostSpawnSystem : DefaultGhostSpawnSystem<NetworkPlayerSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<NetworkPlayerSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetNetworkPlayerComponentPlayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<NetworkPlayerSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
